import numpy as np
from dataset import TCGADataset, TCGA_Outcome, TCGAParameters, TCGAcube_Outcome
...
rs = 1.0
p = 10
params = TCGAParameters(p)
data = TCGADataset(p, params, rs, alpha)
x, t, y = data.GetData()
n = x.shape[0]
x_tes = data.GetTestData()
outcome_model = TCGAcube_Outcome(params, rs)
print('--------------------------------------------------------------------------')
print(args)
...
for i in range(0, repeat_exp):
    ...
    reg = MCDropoutRegressor(x.shape[1], rs, outcome_model, lam, tao)
    ...
    print('Over optimistic', gap[(dm_t2.squeeze() > rs - 0.02) | (dm_t2.squeeze() < 0.02)].sum() / n)
